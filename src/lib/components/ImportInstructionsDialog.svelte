...
<Dialog.Root bind:open>
  <Dialog.Portal>
    <Dialog.Overlay
      class="data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-40 bg-black/80 backdrop-blur"
      data-testid="import-instructions-backdrop"
      on:click={() => (open = false)}
    />

    <Dialog.Content
      class="data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95  fixed left-1/2 top-1/2 z-50 w-[min(92vw,48rem)] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white/10 bg-neutral-900/95 p-6 text-neutral-100 shadow-2xl backdrop-blur"
      role="dialog"
    ...
    >
      {#snippet child({ props, open })}
        {#if open}
          <div {...props} transition:fly={{ y: 5, duration: 150, easing: cubicOut }}>
            ...
            >
              ...
              <section class="space-y-2">
                <h3 class="text-base font-semibold text-neutral-100">
                  Need to add overlays later?
                </h3>
                <p class="text-neutral-300">
                  Drag ROI/annotation JSON files or feature CSVs onto the viewer&mdash;they load
                  without re-importing the whole sample.
                </p>
              </section>
